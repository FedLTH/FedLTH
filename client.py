  ...
  while global_epoch < conf['global_epoch']:
    for client in client_list: 
      ...
      if op=='train':
        ...
        model_para,lr,noise_multiplier=data[0],data[1],data[2]
        #本地训练
        local_model.model.load_state_dict(model_para)
        train_time,weight,acc,loss=local_model.train(train_data,client.train_indices,lr=lr,noise_multiplier=noise_multiplier)
        ...
        # 上传状态字典、loss、test acc、train_time
        send_data(client.sock,[weight,loss, acc, train_time]) 
        comm_size=len(compress(pickle.dumps(weight)))
        client.comm_datasize_list.append(comm_size)
      elif op=='wait':
        global_epoch=recv_data(client.sock)
        print(f'client {client.id} wait')
        client.train_time_list.append(-1)#表示没参加训练
        client.comm_datasize_list.append(-1)#表示没参加训练
        client.loss_list.append(-1)
        client.acc_list.append(-1)
      # 全局模型剪枝后 需要重新分发模型结构
      elif op=='model':
        model=recv_data(client.sock)
        local_model=Local_model(model)
